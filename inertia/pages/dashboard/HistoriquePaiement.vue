          ...
          <div class="border border-blue-400 rounded-lg p-4 flex items-center justify-between w-64 bg-white">
            <div>
              ...
              <div class="text-2xl font-bold text-black">
                XAF {{ totalMontant.toLocaleString() }}
              </div>
            </div>
    ...
    <div class="bg-white rounded-lg shadow overflow-auto">
      ...
      >
        ...
        <div class="w-48 font-bold flex items-center gap-2">
          ...
          <User class="w-5 h-5 text-blue-500" /> Dr. {{ paiement.first_name }}
        </div>
        <div class="w-32 text-gray-600">{{ paiement.last_name }}</div>
        <div class="w-56 text-gray-600">{{ paiement.email }}</div>
        <div class="w-32 text-gray-600">{{ paiement.montant.toLocaleString() }} XAF</div>
    ...
    <div v-if="showDetails" class="fixed inset-0 z-50 flex">
      ...
      <div class="w-full max-w-md h-full bg-white shadow-lg p-8 relative animate-slide-in-right overflow-y-auto">
        ...
        <div v-if="selectedPaiement">
          ...
          <div class="mb-2"><b>Montant :</b> {{ selectedPaiement.montant.toLocaleString() }} XAF</div>
...
paiements.value = data.map(p => ({
  ...
  type: p.type ?? 'Non défini',  // ou p.appointment?.type si ça existe vraiment
  date: new Date(p.datePaiement).toLocaleString('fr-FR'),
  profileImage: p.profileImage || '/doctor1.jpg',
}))
