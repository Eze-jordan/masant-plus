...
export default class NotificationController {
  ...
  public async markAsRead({ params, auth, response }: HttpContextContract) {
    ...
    try {
      ...
      if (!notification) {
        return response.status(404).json({ message: 'Notification non trouvée ou vous n\'êtes pas autorisé à la marquer comme lue' });
      }
      ...
      return response.status(500).json({ message: 'Erreur serveur lors de la mise à jour de la notification' });
    }
  }
